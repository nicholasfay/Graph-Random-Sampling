...
def graphSampleStatistics(origG, sampledG, selected, inFile, w):
    ...
    out_degree = distributionEstimator(
        outDegreeIndicator, origG, sampledG, selected, inFile, w)
    in_degree = distributionEstimator(
        inDegreeIndicator, origG, sampledG, selected, inFile, w)
    plt.figure()
    #plt.yscale('log')
    #plt.xscale('log')
